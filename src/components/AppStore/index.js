...
class AppStore extends Component {
  state = {categoryName: tabsList[0].tabId, searchValue: ''}

  changeSearch = event => {
    this.setState({searchValue: event.target.value})
  }

  clickChangeTabs = value => {
  ...
  render() {
    ...
    const filteredApp = appsList.filter(
      eachValue => eachValue.category === categoryName,
    )
    const searchApp = filteredApp.filter(value => {
      const lowerValue = value.appName.toLowerCase()
      return lowerValue.includes(searchValue.toLowerCase())
    })
        ...
        <ul className="list-prop">
          {tabsList.map(eachValue => (
            <TabItem
              key={eachValue.tabId}
              tabValue={eachValue}
              underline={categoryName}
              clickChange={this.clickChangeTabs}
            />
          ))}
        </ul>
        <ul className="app-list-prop">
          {searchApp.map(eachApp => (
            <AppItem key={eachApp.appId} sendApp={eachApp} />
          ))}
        </ul>
      </div>
    )
